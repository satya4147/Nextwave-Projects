...
import './index.css'
import AppTabItem from "../AppTabItme"
import AppItemDisplay from "../AppItme"
...
class AppStore extends Component{
    ...
    getSearchResults = () =>{
      ...
      const updateResult = appsList.filter(eachItemName => eachItemName.appName.toLowerCase().includes(searchInput.toLowerCase()))

      return updateResult
    }

    getActiveTabApps = serachApp =>{
      const {activeTabId} = this.state

      const fillterApp = serachApp.filter(eachApp => eachApp.category === activeTabId)
  ...
  render(){
      ...
      const filtterTabApp = this.getActiveTabApps(searchResult)
    ...
    return(
      ...
      <h1 className="heading">App Store</h1>
          ...
          {tabsList.map(eachTab =>(
            <AppTabItem TabDetails = {eachTab} key = {eachTab.tabId}
            setActiveApp = {this.setActiveTabApps}
            isActive = {activeTabId === eachTab.tabId} />
          ))}

          </ul>
          <ul className="appTabItem-list">
            {filtterTabApp.map(eachItme =>(
              <AppItemDisplay AppItemsDetails = {eachItme}  />
            ))}
          </ul>
      </div>
      </div>
